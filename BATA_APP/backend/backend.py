...
import csv
from ..models.Bata_model import Incidencia, MonthValues, Operador
import reflex as rx
...
class State(rx.State):
    ...
    def get_current_month_values(self):
        ...
        num_solucionadas = len(
            [incidencia for incidencia in current_month_incidencias if incidencia.status == "Solucionada"]
        )
        num_pendientes = len(
            [incidencia for incidencia in current_month_incidencias if incidencia.status == "Pendiente"]
        )
        num_bitrix = len(
            [incidencia for incidencia in current_month_incidencias if incidencia.status == "Tarea Creada"]
        )
    ...
    def get_previous_month_values(self):
        ...
        previous_month_incidencias = [
            ...
            <= datetime.strptime(incidencia.date, "%Y-%m-%d %H:%M:%S")
            <= last_day_of_last_month
        ]
        num_incidencias = len(previous_month_incidencias)
        num_solucionadas = len(
            [incidencia for incidencia in previous_month_incidencias if incidencia.status == "Solucionada"]
        )
        num_pendientes = len(
            [incidencia for incidencia in previous_month_incidencias if incidencia.status == "Pendiente"]
        )
        num_bitrix = len(
            [incidencia for incidencia in previous_month_incidencias if incidencia.status == "Tarea Creada"]
        )
    ...
    def delete_incidencia(self, id: int):
        ...
        self.load_entries()
        return rx.toast.info(
            f"Incidencia pour {incidencia.name} a été supprimée.", position="bottom-right"
        )
    ...
    def download_csv_data(self):
        ...
        filename = datetime.now().strftime("%Y_%m_%d_%H_%M_%S") + "_incidencias.csv"
        return rx.download(
            data=csv_data,
            filename=filename,
        )
