...
# C'est la classe pour les statistiques dans 
class StatsState(rx.State):
    """C'est la classe pour les statistiques dans le dashboard."""
    ...
    # C'est la fonction pour changer le tab 
    @rx.event
    def set_selected_tab(self, tab: str | list[str]):
        self.selected_tab = tab if isinstance(tab, str) else tab[0]

    # C'est la fonction pour changer le graphique
    def toggle_areachart(self):
        self.area_toggle = not self.area_toggle

    # C'est la fonction pour randomiser les données
    def randomize_data(self):
        # Si les données sont déjà remplies, ne pas randomiser
        if self.users_data:
            return

        for i in range(30, -1, -1):  # Inclure les données d'aujourd'hui
        ...
        # C'est la fonction pour les données des graphiques sur les appareils
        self.device_data = [
            {"name": "Desktop", "value": 33, "fill": "var(--blue-8)"},
...
# C'est un composant pour le bouton de changement de graphique 
